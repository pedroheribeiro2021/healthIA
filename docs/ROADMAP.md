...
**Status:** ✅ implementada na branch `fase-6-metas-relatorios-ia` (234 testes verdes), **nunca mergeada em `main`** — o merge é o primeiro passo da Fase 7. ⚠️ A `NavBar` dessa branch é a versão antiga (11 abas com rolagem horizontal): mergear sem resolver o conflito a favor de `main` regride a navegação corrigida no commit `b16b780`.

## Fase 7 — Rotina, Hábitos e Navegação

Especificação completa: [`FASE-7-ROTINA.md`](FASE-7-ROTINA.md). Conteúdo do plano do Pedro: [`PLANO-SAUDE.md`](PLANO-SAUDE.md).

Motivação (diagnóstico de 30/07/2026, com dado real de produção): seis fases entregues e o app não é usado. Sem dado novo desde 22/07, nenhuma meta cadastrada, dado de teste em produção, a bioimpedância real de 23/07 só no PDF, e 5 das 9 telas escondidas atrás de "Mais". O que falta não é feature — é o app conhecer a **rotina** do Pedro, não só as medições do relógio dele.

- **Etapa 0 — Diagnóstico e higiene:** mergear a Fase 6; investigar por que o sync do Health Connect parou (veredito escrito, sem construir sync novo); remover o dado de teste (exceção única e documentada ao append-only, via SQL administrativo); importar as 6 medições reais de bioimpedância de fev–jul/2026.
- **Etapa 1 — Hábitos, metas e check-in:** tabelas `habits`/`habit_logs` (migration 008) com hábitos derivados de `health_events` onde o dado já existe; seed dos 9 hábitos e das 4 metas do plano (009); calculators de adesão e streak; 4 regras de insight novas; API de hábitos; check-in na home e tela `/plano`.
- **Etapa 2 — Navegação por rotina:** 5 abas (Hoje · Plano · Evolução · Insights · Mais), registro vira FAB em vez de aba, `/evolucao` agrupa Corpo/Sono/Exercícios/Relatórios com redirect das rotas antigas.
